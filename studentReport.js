...
Take up to 10 minutes to write a function called makeStudentReport that takes a single argument, data. data is an array of objects. 
Each object in the array represents
a student and their letter grade for a course — for example, {name: 'Johnny Robot', grade: 'C'}.
...
function makeStudentsReport(data) {
    ...
    for (let i = 0; i < data.length; i++) {
      const item = data[i];
      output.push(`${item.name}: ${item.grade}`);
    }
    return output;
  }
